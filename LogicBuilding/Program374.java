...
class DBMS
{
   public LinkedList <Student> lobj;
    ...
    //  Update student set salary = amount where RID = student_RID
    //  Update student set salary = amount where name = student_name
        ...
        while(true)
        {
            ...
            if(QuerySize == 1)
            {
                if("help".equals(tokens[0]))
                {
                    ...
                    System.out.println("Update salary by student RID :      update student set salary = amount where RID = Student_RID");
                    System.out.println("Update salary by student Name :     update student set salary = amount where name = Student_Name");
                    System.out.println("Delete specific data by RID :       delete from student where RID = Student_RID");
            ...
            else if(QuerySize == 10)
            {
                if("update".equals(tokens[0]))
                {
                    if("salary".equals(tokens[3]))
                    {
                        ...
                        {
                            UpdateSalary(Integer.parseInt(tokens[5]), Integer.parseInt(tokens[9]));
                        }
                        else if("name".equals(tokens[7]))
                        {
                            UpdateSalary(Integer.parseInt(tokens[5]), tokens[9]);
                        }
                    }
                }
            }
    ...
    public void UpdateSalary(int amount, int rid)
    {
        for(Student sref : lobj)
        {
            if(sref.RID == rid)
    ...
    public void UpdateSalary(int amount, String str)
    {
        for(Student sref : lobj)
        {
            if(str.equals(sref.Name))
